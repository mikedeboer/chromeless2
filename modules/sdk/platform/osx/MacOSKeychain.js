...
const {MacTypes} = require("./frameworks/CoreFoundation");
const {Security} = require("./frameworks/Security");
const {KeychainServices} = require("./KeychainServices");
 ...
 */
var MacOSKeychain = exports.MacOSKeychain = {
...
MacOSKeychain.verifySignature = function() {
  ...
  try { // These APIs were only added in OS X 10.6
    ...
    Logger.warning('The application binary\'s signature cannot be verified; ' +
                   'Keychain services may not function properly or you may be ' +
                   'prompted repeatedly to allow access. Try upgrading your ' +
                   'application to the newest version or deleting and reinstalling ' +
                   'the application.');
    return false;
  } catch (e) {
...
MacOSKeychain.verifyConfiguration = function(logWarnings) {
  ...
  if (! Preferences.mozilla.rememberSignons.value) {
    if (logWarnings)
      Logger.warning('The preference signon.rememberSignons is set to false. ' +
                     'Passwords will not be stored.');
  ...
  if (! Preferences.mozilla.autofillForms.value) {
    if (logWarnings)
      Logger.warning('The preference signon.autofillForms is set to false. ' +
                     'Password fields will not be filled with stored passwords.');
  ...
  if (Preferences.mozilla.sanitizeOnShutdown.value
    ...
    if (logWarnings)
      Logger.warning('The preferences privacy.sanitize.sanitizeOnShutdown and ' +
                     'privacy.clearOnShutdown.passwords are set to true. ' +
...
MacOSKeychain.confirmRemoveAll = function() {
  ...
  if (Preferences.allowRemoveAll.hasUserValue()) {
    ...
    Logger.warning('Requested to remove all logins while shutting down but user ' +
                   'has not explicitly enabled this behaviour for Keychain ' +
                   'passwords. Cannot safely prompt the user now, so ignoring ' +
                   'the request.');
    return false;
  } else {
...
//          * Services.prompt.BUTTON_TITLE_IS_STRING);
    ...
    var result = Services.prompt.confirmEx(null,
        ...
        Services.appinfo.name + " is asking to clear all saved passwords. This " +
        "is not recommended when storing passwords in your keychain, since these " +
        "passwords may be needed by other applications on your computer.",
...
MacOSKeychain.confirmSanitizePasswords = function(showCancel) {
  ...
  if (! (Preferences.mozilla.sanitizeOnShutdown.value
    ...
    var result = Services.prompt.confirmEx(null,
        ...
        'You have configured ' + Services.appinfo.name + ' to clear saved passwords ' +
        'when shutting down. This is not recommended when storing passwords in ' +
        'your keychain, since these passwords may be needed by other applications ' +
        'on your computer.',
