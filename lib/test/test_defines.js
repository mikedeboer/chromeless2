...
describe("ifdef parser", function() {
  ...
  it("should correcly parse OSX ifdefs", function(next) {
    ...
    Defines.parseFile(Path.join(fixtures, "defines.html"), defines, function(err, content) {
      Assert.ok(!err);
  ...
  it("should correctly parse define statements", function(next) {
    Defines.parseFile(Path.join(fixtures, "defines_extra.html"), {}, function(err, content) {
      Assert.ok(!err);
