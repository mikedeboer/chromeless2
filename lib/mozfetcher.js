...
this.needsFetch = function(callback) {
  ...
  Mkdirp(self._cacheDir, function(err) {
    ...
    self.downloadArchive(function(err) {
      ...
      var extractedMd5 = Path.dirname(path) + ".md5";
      var readedMd5;
...
this.fetchIfNeeded = function(callback) {
  ...
  self.needsFetch(function(err, needsFetch) {
    ...
    if (!needsFetch)
      return callback();
    new Moztarball(self._buildDir).unpack(self.getArchivePath(), callback);
  });
};
...
this.download = function(url, tarball, callback) {
  ...
  Http.get(url, function(res) {
    ...
    var progressBar = new ProgressBar('  Downloading [:bar] :percent :etas', { total: parseInt(size) });
