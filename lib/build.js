...
exports.appify = function(executionMode, xulrunner, appToLaunch, buildDir, multi, callback) {
  ...
  Fs.readFile(Path.join(appCodeDir, "appinfo.json"), {encoding: "utf8"}, function(err, data) {
    if (err)
      return callback(err);

    var appInfo = JSON.parse(data);
    appInfo.shortname = Path.basename(appCodeDir).toLowerCase();
    ...
    require("./build/" + platform).outputAppShell(appCodeDir, appInfo, buildDir, devMode, function(err, params) {
      if (err)
        return callback(err);

      self.outputXulApp(params.xulrunnerAppDir, xulrunner, appToLaunch, appInfo,
                        params.defines, devMode, callback);
    });
  });
};
    ...
    Fs.mkdir(moduleTargetDir, function(err) {
      ...
      Async.eachSeries(modules,
        function(moduleName, nextModule) {
          ...
          var preprocessRegex = createFilePatternRegExp(moduleOpts.preprocessRegex);
              ...
              function(include, nextInclude) {
                ...
                Async.eachSeries(toPreprocessList,
                  function(toPreprocess, nextFile) {
                    toPreprocess = Path.join(src, toPreprocess);
                    var preprocessDst = Path.join(dst, toPreprocess);
                    Defines.parseFile(toPreprocess, defines, devMode, function(err, content) {
                      if (err)
                        return nextFile(err);
