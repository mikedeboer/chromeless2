...
class OSAppifier(object):
    ...
    def output_app_shell(self, browser_code_dir, dev_mode, verbose=True):
        ...
        xul_bin_src = os.path.join(self.dirs.build_dir, "xulrunner-sdk", "bin", "XUL.framework", "Versions", "Current")

        # and recursively copy in the bin/ directory out of the sdk 
        if verbose:
        ...
        # since OSX 10.9 it's also necessary to place a copy Info.plist in the
        # Framework dir.
        info_plist_path = os.path.join(cur_ver_dir, "Info.plist")
        self._sub_and_copy(template_path, info_plist_path, {
                "application_name": app_info.name,
                "short_version": app_info.version,
                "full_version": (app_info.version + "." + app_info.build_id)
